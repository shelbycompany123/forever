...
const OrderDetails = () => {
  ...
  const getTotalAmount = () => {
    ...
    return order.items.reduce((total, item) => {
      return total + ((item.new_price || item.price) * item.quantity)
    }, 0)
  }
            ...
            <div className='flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4'>
              <div>
                ...
                <div className='flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-sm text-gray-600'>
                  <span>📅 Đặt ngày: {new Date(order.date).toLocaleDateString('vi-VN')}</span>
              ...
              <div className={`flex items-center gap-2 px-4 py-2 rounded-full border ${getStatusColor(order.status)}`}>
                ...
                <span className='font-semibold'>{getStatusName(order.status)}</span>
              </div>
            </div>
          </div>
          ...
          <div className='p-6'>
            ...
            <div className='space-y-4'>
              {order.items.map((item, idx) => (
                <div key={idx} className='flex items-start gap-4 p-4 rounded-lg border border-gray-100'>
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <div className='mt-2 text-sm text-gray-500'>
                      Tổng: {formatCurrency((item.new_price || item.price) * item.quantity)}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className='mt-6 pt-6 border-t border-gray-200'>
              <div className='flex justify-between items-center text-lg font-semibold text-gray-900'>
                <span>Tổng cộng:</span>
                <span className='text-2xl text-green-600'>{formatCurrency(getTotalAmount())}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className='flex flex-col sm:flex-row gap-4'>
          ...
          {order.status === 'chua_xac_nhan' && (
            <button
              onClick={async () => {
                try {
                  const options = confirm('Bạn có chắc chắn muốn huỷ đơn này không?');
                  if (options) {
                    await axios.post(
